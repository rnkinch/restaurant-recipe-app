    ...
    const filtered = recipes.filter(recipe =>
      ...
      ((recipe.ingredients && Array.isArray(recipe.ingredients)) &&
        recipe.ingredients.some(ingredient =>
          ...
          (ingredient.purveyor && ingredient.purveyor.name && ingredient.purveyor.name.toLowerCase().includes(lowerQuery))
        )) ||
  ...
  return (
    <Router>
      <Navbar bg="light" expand="lg" className="mb-3 shadow-sm">
        <Container fluid>
          <Navbar.Brand as={Link} to="/">XYZCompany Recipe and Plating Guide</Navbar.Brand>
          <Navbar.Toggle aria-controls="basic-navbar-nav" />
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="ms-auto">
              <Nav.Link as={Link} to="/">Recipes</Nav.Link>
              <NavDropdown title="Reports" id="reports-dropdown">
                <NavDropdown.Item disabled>More Reports Coming Soon</NavDropdown.Item>
              </NavDropdown>
      ...
      <Container fluid>
        <Row>
          <Col md={2} className="bg-light p-0" style={{ minHeight: 'calc(100vh - 56px)', overflowY: 'auto' }}>
            <h4>Recipe List</h4>
            <Nav className="flex-column" style={{ fontSize: '0.84375rem !important' }}>
              {recipes.map(recipe => (
                <Nav.Link
                  key={recipe._id}
                  as={Link}
