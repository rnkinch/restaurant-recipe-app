...
const PdfEditor = ({ recipe }) => {
  ...
  useEffect(() => {
    ...
    const defaultFields = [
      ...
      {
        id: 'allergens',
        content: Array.isArray(recipe?.allergens) ? recipe.allergens.join(', ') : 'No allergens',
        x: 450,
        y: 30,
        fontSize: 12,
        isBold: false,
      ...
      {
        id: 'serviceTypes',
        content: Array.isArray(recipe?.serviceTypes) ? recipe.serviceTypes.join(', ') : 'No service types',
        x: 450,
        y: 80,
        fontSize: 12,
        isBold: false,
    ...
    const fetchTemplate = async () => {
      try {
        ...
        console.error('Failed to load default template:', err.message);
        setFields(defaultFields);
      }
    };
  ...
  const handleSave = useCallback(async () => {
    try {
      console.log('Saving default template:', JSON.stringify(fields, null, 2));
      ...
      console.error('Save default failed:', err.message);
  ...
  const handleResetToDefault = useCallback(async () => {
    try {
      ...
      console.error('Reset to default failed:', err.message);
