...
const ActiveRecipesReport = () => {
  ...
  const handlePrintPreview = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Active Recipes Report - Print Preview</title>
          <style>
        ...
        <body>
          ...
          <table>
            ...
            <tbody>
              ${recipes.length === 0 ? '<tr><td colspan="4">No active recipes found.</td></tr>' : 
                recipes.map(recipe => `
                  <tr>
                    <td>${recipe.name}</td>
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', 'active_recipes_report.csv');
